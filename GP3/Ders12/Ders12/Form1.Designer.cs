namespace Ders12
{
    partial class Form1
    {
        /// <summary>
        ///  Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        ///  Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        ///  Required method for Designer support - do not modify
        ///  the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            label1 = new Label();
            nudSayi = new NumericUpDown();
            btnBasla = new Button();
            ((System.ComponentModel.ISupportInitialize)nudSayi).BeginInit();
            SuspendLayout();
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Location = new Point(12, 9);
            label1.Name = "label1";
            label1.Size = new Size(87, 15);
            label1.TabIndex = 0;
            label1.Text = "Öğrenci &Sayısı: ";
            // 
            // nudSayi
            // 
            nudSayi.Location = new Point(105, 7);
            nudSayi.Maximum = new decimal(new int[] { 200, 0, 0, 0 });
            nudSayi.Minimum = new decimal(new int[] { 1, 0, 0, 0 });
            nudSayi.Name = "nudSayi";
            nudSayi.Size = new Size(87, 23);
            nudSayi.TabIndex = 1;
            nudSayi.Value = new decimal(new int[] { 1, 0, 0, 0 });
            // 
            // btnBasla
            // 
            btnBasla.Location = new Point(198, 7);
            btnBasla.Name = "btnBasla";
            btnBasla.Size = new Size(75, 23);
            btnBasla.TabIndex = 2;
            btnBasla.Text = " >";
            btnBasla.UseVisualStyleBackColor = true;
            btnBasla.Click += btnBasla_Click;
            // 
            // Form1
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            AutoScroll = true;
            ClientSize = new Size(418, 239);
            Controls.Add(btnBasla);
            Controls.Add(nudSayi);
            Controls.Add(label1);
            Name = "Form1";
            Text = "Form1";
            ((System.ComponentModel.ISupportInitialize)nudSayi).EndInit();
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private Label label1;
        private NumericUpDown nudSayi;
        private Button btnBasla;
    }
}
